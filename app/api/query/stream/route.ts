...
export async function POST(request: Request) {
  ...
  const stream = new ReadableStream({ async start(controller) { try {
    ...
    const groqResponse = await fetch('https://api.groq.com/openai/v1/chat/completions', { method: 'POST', headers: { Authorization: `Bearer ${groqApiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: groqModel, temperature: 0.1, max_tokens: 900, messages: [{ role: 'system', content: 'You are DocuTrust policy assistant. Answer only from the supplied policy passages. Return clean plain text with a short direct answer first, then a small section titled "Key rules" with 2-5 bullets when useful. Do not output Markdown emphasis, HTML tags, pipe characters, extraction artifacts, line-reference citations, or raw document text. Cite only with [1], [2]. If evidence is insufficient, say so clearly.' }, { role: 'user', content: `Question: ${query}\\n\\nPolicy passages:\\n${context}` }] }) })
